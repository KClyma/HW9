...
class Link():
    def __init__(self, name="", node1="1", node2="2", length=None, angleRad=None, width=1.0, thickness=0.25, material="Steel"):
        self.name = name
    ...
    def computeWeight(self):
        """
        computes the weight of the link based on length, cross-sectional area, and material density
        :return: weight of the link in pounds (lb)
        """

        density = 0.284 if self.material.lower() == "steel" else 0.1  # lb/in^3
        area = self.width * self.thickness
        return self.length * area * density  # in units of pounds (lb)
...
class TrussModel():
    ...
    def getSupportReactions(self):
        """
        returns vertical support reactions based on total truss weight
        :return: left and right vertical reaction forces
        """
        try:
            total_weight = sum(l.computeWeight() for l in self.links if l.length is not None)
            return total_weight / 2.0, total_weight / 2.0
        except Exception as e:
            print("Error computing support reactions:", e)
            return 0.0, 0.0
...
class TrussView():
    ...
    def drawLinks(self, truss=None):
        ...
        for l in truss.links:
            ...
            weight = l.computeWeight() if hasattr(l, 'computeWeight') else 0.0

            # build a tool tip string
            st = (f"Link: {l.name}\n"f"Start Node: {l.node1_Name}\n"f"End Node: {l.node2_Name}\n"f"Length: {l.length:.2f} in\n"f"Angle: {l.angleRad:.2f} rad\n"f"Width: {l.width:.2f} in\n"f"Thickness: {l.thickness:.2f} in\n"f"Material: {l.material}\n"f"Weight: {weight:.2f} lb")
    ...
    def drawNodes(self, truss=None, scene=None):
        ...
        Ly, Ry = truss.getSupportReactions()

        for n in truss.nodes:
            x = n.position.x - offset.x
            y = (n.position.y - offset.y)
            toolTip = "Node: " + n.name
            # Draw the left support as a pin and the right support as a roller, with vertical reactions in the tooltip.
            if n.name.lower() == 'left':
                toolTip += f"\nVertical Reaction: {Ly:.2f} lb"
                n.graphic = RigidPivotPoint(x, -y, 10, 18, brush=self.brushPivot, name=n.name)
                self.scene.addItem(n.graphic)

            elif n.name.lower() == 'right':
                toolTip += f"\nVertical Reaction: {Ry:.2f} lb"
                self.drawACircle(centerX=x, centerY=y, Radius=8,pen=self.penNode,brush=self.brushNode,name="Roller: " + n.name,tooltip=toolTip)
            # self.drawACircle(centerX=x,centerY=y,Radius=8,pen=self.penNode,brush=self.brushNode, name=toolTip, tooltip=toolTip)
            self.drawALabel(x=x - 5, y=y + 15, str=n.name, pen=self.penLabel, tip=toolTip)
        pass
...
class TrussController():
    ...
    def getTotalWeight(self):
        return sum(link.computeWeight() for link in self.truss.links)

    def getSupportReactions(self):
        W = self.getTotalWeight()
        return W / 2, W / 2  # assuming symmetric load
    ...
    def installEventFilter(self, filterObject):
        self.view.scene.installEventFilter(filterObject)
